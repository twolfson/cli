...
// Overwrite `_findMigrations` to require non-duplicate ordered migrations
// https://github.com/sequelize/umzug/blob/v1.11.0/index.js#L408-L438
var _findMigrations = Umzug.prototype._findMigrations;
Umzug.prototype._findMigrations = function () {
  // Find our original migrations
  console.log('hey');
  return _findMigrations.call(this)
    .then(function (migrations) {
      // Extract the migration id (e.g. 0001-foo-bar -> 0001)
      var migrationIds = migrations.map(function extractId (migration) {
        var match = migration.file.match(/^(\d+)-/);
        assert(match, 'Unable to extract identifier from "' + migration.file + '". ' +
          'Please verify it follows the format "0001-my-migration.js"');
        return match[1];
      });

      // Count up our ids and pick out the duplicates
      // `0004-foo`, `0005-bar`, `0005-baz` -> {0004: 1, 0005: 2)
      var migrationIdCounts = _.countBy(migrationIds);
      // {0004: 1, 0005: 2) -> {0005: 2}
      var duplicateIdCounts = _.pickBy(migrationIdCounts, function hasDuplicateId (value, key) {
        return value >= 2;
      });
      if (!_.isEmpty(duplicateIdCounts)) {
        var duplicateIds = Object.keys(duplicateIdCounts);
        throw new Error('Duplicate migration identifiers found: ' + JSON.stringify(duplicateIds) + '. ' +
          'Please verify all migrations are in sequential order');
      }

      // Return our migrations
      return migrations;
    });
};
