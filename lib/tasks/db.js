...
Umzug.prototype._findMigrations = function () {
  ...
  return _findMigrations.call(this)
    .then(function (migrations) {
      // Verify we have no migrations with the same id (e.g. `0001` in `0001-foo`)
      var migrationIdToIndexMap = {};
      migrations.forEach(function verifyNoDuplicateId (migration, index) {
        // Extract our identifier
        var currentFile = migration.file;
        var match = currentFile.match(/^(\d+)-/);
        assert(match, 'Unable to extract identifier from "' + currentFile + '". ' +
          'Please verify it follows the format "0001-my-migration.js"');
        var id = match[1];

        // If the id has been seen before, then raise an error
        if (migrationIdToIndexMap[id] !== undefined) {
          var originalIndex = migrationIdToIndexMap[id];
          var originalFile = migrations[originalIndex].file;
          throw new Error('Migrations "' + originalFile + '" and "' + currentFile + '" ' +
            'have the same id "' + id + '". Please update the identifiers to prevent duplicate ids.');
        // Otherwise, save it
        } else {
          migrationIdToIndexMap[id] = index;
        }
      });
